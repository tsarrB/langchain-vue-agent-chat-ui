...
>(
    ...
    return getMessages(historyValues.value).map((message: any, idx: number) => {
      ...
      let messageBranch =
        firstSeen && firstSeen.checkpoint
          ? branchData.value.branchByCheckpoint[
              firstSeen.checkpoint.checkpoint_id
            ]
          : undefined;
  ...
  const submit = async (
    ...
    try {
      ...
      const checkpoint =
        submitOptions?.checkpoint ?? threadHead.value?.checkpoint ?? undefined;
      if (checkpoint != null) {
        // @ts-ignore
      ...
      const newPath = submitOptions?.checkpoint?.checkpoint_id
        ? branchData.value.branchByCheckpoint[
            submitOptions?.checkpoint?.checkpoint_id
          ]?.branch
        : undefined;
            ...
            mutate: (update: any) => {
              ...
              streamValues.value = {
                ...
                ...(typeof update === "function"
                  ? update(prev as TValues)
                  : update),
              };
            },
          });
        if (event === "metadata") options.onMetadataEvent?.(data);
        if (event === "values") streamValues.value = data;
        if (event === "messages") {
  ...
  const getMessagesMetadata = (message: any, index?: number) => {
    trackStreamMode("messages-tuple", "values");
    return messageMetadata.value?.find(
